...
tibble(both_all = pmax(1/or_all, or_all)) %>% 
  ggplot() + geom_histogram(aes(x=both_all)) + 
  theme_bw(base_size=16) + ylab("Count") + xlab("Odds Ratio") +
...
max(1/min(or_all), max(or_all))
quantile(pmax(1/or_all, or_all), 0.95)
...
## And R^_{Y ~ X} givne all covariates without benchmark
...
for(m in c()) {
  ...
  stan_results <- sm$sampling(data_list)
  saveRDS(stan_results, file=sprintf("drinking_samples_weduc_rank%i.RDS", m))
}

## The rest of the results proceed with rank 5 samples
...
```{r}
## Compute Lambda as a function of R2, find the closest to 3.5
