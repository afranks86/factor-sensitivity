...
cal_tau_cali_wNC <- function(df, R2_no_nc=0.0, R2_nc=R2_no_nc, NC_index, data_list, 
                             treatment_index=1, 
  ...
  if(full_rank) {
    ...
    Gamma_tilde <- df %>% filter(J==treatment_index) %>% pull(B) %>% matrix(dat=., nrow=data_list$K, ncol=m, byrow=TRUE) * sdy
    lambda <- df %>% 
      filter(J==treatment_index) %>% 
      filter(M==1) %>% 
      pull(lambda) * sdy %>% diag
  }
...
get_beta_w_norm <- function(stan_results, data_list, NC_index = 1,
                ...
                probs=NULL, full_rank=FALSE, R2_no_nc=0.0, R2_nc=R2_no_nc, r2y=NULL) {
  ...
  beta_w_norm <- nest_df %>% 
    mutate(data = purrr::map(data, function(
      d) cal_tau_cali_wNC(d, R2_no_nc=R2_no_nc, R2_nc=R2_nc, NC_index=NC_index, 
                          treatment_index = 1, data_list=data_list, 
                          marginal_sensitivity_param = marginal_sensitivity_param, 
                          probs=probs, full_rank=full_rank, r2y=r2y))) %>%
    unnest(cols = c(data))
